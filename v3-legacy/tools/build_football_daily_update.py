...
def eligible_competitive_row(row: dict[str, str]) -> bool:
    # Keep the complete adult-competition roster even when odds are not yet
    # available.  Market completeness is a downstream funnel gate, not a
    # reason to delete the locked match from the day's board.
    return (
...
def main() -> int:
    ...
    audit_script = Path(r"D:\codex\v3_legacy\tools\audit_asian_intent_history.py")
    ...
    micro_edge_script = Path(r"D:\codex\v3_legacy\tools\build_micro_region_tag_edge.py")
    if micro_edge_script.exists() and frozen_lock_available:
        ...
        if spec and spec.loader:
            ...
            micro_module.main()
    elif micro_edge_script.exists():
        print("regional_risk=skipped_no_frozen_plan; existing frozen risk inputs retained")

    bettable_stats_script = Path(r"D:\codex\v3_legacy\tools\build_bettable_event_stats.py")
    ...
    dashboard_script = Path(r"D:\codex\v3_legacy\tools\build_football_dashboard.py")
    ...
    publish_script = Path(r"D:\codex\v3_legacy\tools\publish_football_dashboard_to_github.py")
