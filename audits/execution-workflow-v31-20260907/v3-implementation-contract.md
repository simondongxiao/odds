# 亚盘量化 v3.1：数据、信号、风控与执行契约

版本：asian-risk-v3.1-20260907。适用于新决策；不重写历史计划。核心规则见[选边风控标准](asian-side-risk-v3.md)。这是可以交给开发人员实施的工作流，不代表已开通飞书或真实下单服务。
...
| 原标签 | 初始影子规则 | 输出 |
|---|---|---|
| 阻上/诱下 | 上盘Delta在-8至-3个百分点，基本面支持穿盘、实际阻力有证据、无已证实“热上未阻住”；两个机构方向一致，至少两次可比快照间隔>=5分钟 | 阻上交叉验证候选，继续检查EV及风控；不是直接投注 |
...
当前状态：Skill规范与上述计算/检查模块已升级；尚未把现有日更采集器、HTML比赛行和飞书同步器迁移为v3.1。公开的工作流页是设计与验收说明，不是实时投注清单。不能声称已解决周末亏损或已获得可验证的胜率提升。
