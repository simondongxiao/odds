...
1. **V3 first, V4 second**: settle prior dates and refresh the current list date, then generate the frozen V3 Production result from the V3 Legacy/bridge path. V4 runs independently from its own shadow inputs and may not recreate, overwrite, or reinterpret V3 Production.
2. **Same artifact contract, separate namespaces**: V4 must generate the same daily artifact classes that V3 historically generated, including the daily report, simulation/current-model file, grouped review, bettable-event statistics, per-match detail ledger, frozen pre-match decision ledger, historical settlement audit, and dashboard date data. V3 and V4 files must live in separate version directories or carry an explicit version suffix; they must never overwrite each other. V3 keeps the legacy filenames and formats unchanged for compatibility.
3. **Required daily artifact families**:
   - V3: `reports/`, `simulations/`, `reviews/grouped_edge_review/`, `ledger/bettable_event_stats_YYYY-MM-DD.csv`, `ledger/bettable_event_detail_YYYY-MM-DD.csv`, frozen decision/signal files, settlement audit, and dashboard/date JSON.
   - V4: the same artifact family under `v4_shadow/` or with a `_v4`/`v4_` marker, with `SHADOW_ONLY` and `real_money=false` visible in every user-facing report and dashboard output. V4 A/B/C/N counts must come only from the selected list date's forward-shadow ledger, never from historical replay totals.
4. **List-date roster lock**: both versions use the same immutable Titan007/球探 `list_date` roster. A later refresh may append odds, evidence, score, status, settlement, and review fields, but may not delete rows or reassign a match to another list date. Roster shrink blocks publication.
5. **Decision immutability**: once a match has kicked off or is otherwise past its scheduled kickoff, both V3 and V4 pre-match conclusions are permanently frozen. A new day update may only fill or correct, with provenance, `比分`, `状态`, `结算标签`, `红/红半/走水/黑半/黑`, `实际盈亏`, `比分来源`, `结算时间`, and post-match review notes. It must not recompute or change the prior day's action, selected team, side, line, water, intent, probability, threshold, grade, Kelly/stake, or decision timestamp.
   - **Same-list-date refresh lock**: on the second and every later refresh of the same `list_date`, V4 must merge the prior shadow output before rendering or exporting. Any already-started, live, settled, or cancelled row must remain present, especially prior V4 `A/B/C` rows. If the latest runner returns only not-yet-started rows or returns an empty subset, the prior started rows are reattached with their original grade, candidate team, side, line, water, probability and EV; only post-match status, score, settlement and PnL may be appended. `A/B/C` must never be replaced by `N`, `MISSING`, or disappearance because a later refresh omitted the row.
  6. **Prior-day correction rule**: if an old score or settlement is wrong, correct the result field append-only with the new source and audit note, then recalculate only settlement and PnL columns. Never rerun the prior day's V3/V4 selection using today's odds, today's statistics, today's prior, or today's model parameters.
     - **V4 result backfill lock**: every V4 daily update must load the immediately preceding `list_date`'s frozen V4 Shadow decisions and append verified score, match status, settlement label, PnL, result source, and settlement timestamp to the prior-day record. It must not recalculate or overwrite `grade`, `A/B/C/N`, candidate team, side, line, water, EV, probability, rank, decision timestamp, or any other pre-match field. The prior-day V4 grade is an immutable field: an existing `A`, `B`, `C`, or `N` remains exactly the same after settlement; a missing pre-match grade remains null and cannot be promoted by result backfill. Before publishing, compare grade counts and grade-by-match-id before/after; any difference is `V4_GRADE_IMMUTABILITY_ERROR` and blocks publication.
7. **Cross-version reconciliation**: every update must report, separately for V3 and V4, roster count, computed count, bettable/half-bettable or A/B/C/N counts, settled count, pending/missing count, and publication status. V4 must never turn a V3 missing feed into `V3可投=0`; V3 Production remains the formal decision source.
8. **GitHub delivery is part of completion**: after both versions and settlement append operations pass reconciliation, rebuild the slim dashboard and sync the versioned reports, ledgers, date JSON, and model-status files to `D:\codex\outputs\football_odds_trader\github_publish\odds`. Commit and push to the configured GitHub remote. Return the actual short commit hash and complete cache-busted Pages URL. If push fails, mark the update incomplete and report the exact failure; do not claim GitHub was updated.
9. **No silent fallback**: missing V3 or V4 output is reported as `V3_PRODUCTION_FEED_MISSING` or `V4_SHADOW_PIPELINE_MISSING`, not converted into zero bettable/A/B counts. A zero count is valid only after the corresponding roster, input, computation, and funnel reconciliation has passed.

10. **Mandatory yesterday performance and timestamped pick exports**: every completed daily update must settle the immediately preceding `list_date` using its frozen V3 and V4 pre-match decisions, then report V3 and V4 separately with settled count, `W/HW/P/HL/L`, effective win rate, flat-stake PnL, and ROI. The denominator is the frozen same-day bettable population; never use a newly recomputed or settled-only population. In the same run, write immutable, timestamped bettable lists for both versions, even when a list is empty:
   - V3: `ledger/daily_bettable/v3/bettable_YYYY-MM-DD_<YYYYMMDD_HHMMSS>.csv`
   - V4: `ledger/daily_bettable/v4/bettable_YYYY-MM-DD_<YYYYMMDD_HHMMSS>.csv`
   Each row must preserve `list_date`, `match_id`, competition, Beijing kickoff, home/away teams, action/grade, selected team, giving/receiving side, line, water, probability/EV where available, `decision_at`, `rule_version`, and source snapshot. The file is a frozen pre-match export: later updates may append settlement/status/PnL in a separate record but may not rewrite its decision fields. The user-facing completion block must include the two file paths and the previous-day V3/V4 performance summary; if settlement or either export fails, the daily update is incomplete.
...
### AFC and Asian Games Competition Isolation (2026-09-14)

`亚冠精英`/`AFC Champions League Elite`, `亚冠2`/`AFC Champions League Two` and any remaining AFC Champions League aliases are independent Asian continental scopes: `亚冠精英独立赛事`, `亚冠2独立赛事` and `亚冠独立赛事`. They must never be folded into `欧洲非五大`, `亚洲其他` or a domestic league residual. `亚运会`, `亚运男足`, `亚运女足` and `Asian Games` football are a separate scope: `亚运会足球独立赛事`. Both AFC and Asian Games football enter the daily roster and the V3 bettable/no-bet funnel when they are official competition rows; they are not silently dropped because they are not domestic leagues. Missing odds, evidence or history remains a visible downstream gap and is not a reason to delete the roster row. For review/backtest, settle and reverse-check these scopes independently; do not borrow a different Asian, European or generic residual league prior. The single classifier is `D:\codex\tools\football_competition_normalizer.py`, with the legacy compatibility wrapper at `D:\codex\v3_legacy\tools\european_competition_scope.py`.
...
For implementation and HTML/Feishu integration, also read [数据契约、执行状态与验收](references/v3-implementation-contract.md). This defines the four modules, typed fields, as-of evidence, conservative sizing, idempotent execution, and rollout gates. Use `asian-risk-v3.2.1-20260908` for new reference calculations. A documented gate is not a deployed collector or order service.

**Late-price sensitivity (latest):** read [临场改向与被动调价审查](references/market-move-weekend.md). Restore immediate response to one new comparable pre-match water/line quote; remove the mandatory two-new-snapshot/five-minute hysteresis. Complete upper-to-lower or lower-to-upper reversals are allowed, but a price-triggered new-side recommendation requires independently evidenced resistance against the observed public direction and the full new-side funnel. Passive public-driven repricing, conflicting or missing evidence cannot certify a reversal. Preserve the old decision and separately mark current execution pending; do not blindly execute the old price or auto-buy the opposite. No actual flow alone still does not veto an initial AH analysis: independently sourced qualitative public evidence may be used without invented money shares. Measured same-market money share must exceed its theoretical baseline by strictly more than 5 percentage points to be labeled quantitatively overheated. Historical/in-play decisions and list_date remain frozen. The tested reference reassessment wrapper is `scripts/market_move_guard.py`; live collector/legacy-card adapter migration remains separately tracked.

**Weekend tightening withdrawn (2026-09-08):** do not add 10-15%, multiply thresholds by 1.10-1.15, lower stakes, veto favorites or favor underdogs solely because kickoff is on Saturday/Sunday. Restore `Threshold = 1/(1+Water) + Safety_Buffer` for all days. Weekend/weekday is an audit grouping only. Keep evidence-based cup, rotation, EV, cooling and concentration controls unchanged; no blind calendar tightening without a new user-approved, validated rule.

**Cup/rotation pre-Delta gateway:** before any new Delta calculation or executable intent approval, read [赛制、主客场与轮换网关](references/cup-rotation-gateway.md) and apply `scripts/cup_rotation_gateway.py`. In a second leg, join first-leg goals by tie ID and team IDs; a first-leg leader with a margin >=2 blocks the entire match as `强制跳过（警惕轮换与功利控盘）`, regardless of Delta, and never auto-flips to the other team. A favorite giving 0.75-1.0 with two/three matches in the rolling week and high rotation risk defaults to skip; explicit quarter mode imposes an absolute 0.25 standard-unit cap after the original strategy. Missing format/leg/team/venue evidence is pending, not a zero lead or low risk. Confidence scores may be discounted by motivation, venue-specific fatigue and rotation, but historical win rates stay immutable and `必须全力争胜` is not a probability bonus. The callable gateway is integrated before Delta in the new wrapper; legacy daily collectors/decisions still require adapter migration.
...
- **Snapshot Lock / List-Date Immutability is mandatory.** `list_date` is a permanent roster key, not a derived display date. Once a match is first stored under a Titan007/球探 list date such as `2026-09-04`, that match must stay under `2026-09-04` forever, even if its Beijing kickoff is after midnight or the match later appears in another source page. Do not reassign it to a later natural date.
- **Refreshes are append/status updates, not full overwrites.** For any historical `list_date` earlier than the current update date, scripts may update only score, match status, settlement label, red/black/half/push result, PnL, and audit notes. They must not delete the row, change pre-match Asian intent, candidate tag, final direction, bettable/not-bettable status, selected team, line, water, odds snapshot timestamp, Kelly, or stake decision.
- **Per-date roster lock is a hard gate.** After the first successful fetch for list date `D`, save a roster of every covered `match_id`, Chinese match name, league, kickoff time, and source. Later runs must merge against that roster. If the live source no longer returns a roster match after kickoff or because of source omission, keep the prior row and mark it `本次未刷新到-保留上一版快照`; never render the date as empty or shrink the bettable list because the latest fetch missed rows.
- **Bettable history is frozen independently of current recomputation.** For past dates, dashboard filters, exports, settlement reviews, and GitHub pages must use frozen/page-level bettable records from that original date. If an old dashboard predates the `frozen_bettable` field, rebuild that old date only from the timestamped dashboard snapshot or same-day ledger and label it as `旧版快照恢复`; do not use today's model statistics to erase or replace historical bettable status.
- **Count-shrink audit is required.** If a refreshed date's total rows or bettable rows falls materially from the last known value, such as `103 -> 40` or `>0 -> 0`, treat it as a pipeline error until proven by explicit cancellation/removal records. The script must retain the last locked roster and surface a warning instead of publishing the shrunken view as normal.
- **Archive pages must be durable.** The main dashboard may contain date tabs, but every list date must also be preserved as a static archive snapshot under `dashboard/snapshots/YYYY-MM-DD_HHMMSS/` or an equivalent archive path. Publishing to GitHub must copy those archives so historical audit links keep working.
...
- Immutable started/settled decision lock: once a match is `已开赛`, `进行中`, `完场`, `已结算`, or otherwise past its scheduled kickoff, its pre-match conclusion is frozen forever. Later updates may only append or update `比分`, `状态`, `结算标签`, `红/红半/走水/黑半/黑`, `实际盈亏`, `比分来源`, `结算时间`, and post-match review notes. Later updates must not modify the frozen `亚盘意图`, `候选标签`, `盘口档位`, `即时盘口/水位`, `正向/反向结论`, `可投/半仓可投/不投`, `投注球队`, `投注盘向`, `综合胜率`, `通过阈值`, `Kelly/仓位`, or pre-match timestamp. If a new/live/post-match snapshot produces a different conclusion, store it only as `赛后快照差异` or `结论漂移`, and never overwrite the original pre-match ledger or the dashboard red EV conclusion for audit.
- Rollback discipline: when the user asks to return to a previous version because a later refresh dropped already-started or settled bettable rows, restore the prior pre-match bettable ledger as a frozen ledger, then rerun only score/status/settlement/PnL. Do not regenerate those rows' betting conclusions from current odds or final scores.
...
- The red EV badge and the bettable filter must share one final decision function. If the selected date's bettable filter includes a match, the selected-match red EV badge must show the same `可投/半仓可投` action, the concrete positive-expectation team, and the direction (`正向/反向`, `上盘/下盘`). It must not display `不投：已开赛/完场` for a row that the filter keeps as bettable; state may only be appended as review/execution context.
...
- `bettable_event_detail_YYYY-MM-DD.csv` may be a long-run settled walk-forward detail file. Unless it explicitly contains the selected list date plus a same-day freeze timestamp/source field, it must not replace the frozen same-day bettable ledger for yesterday/today review. Dashboard builders and review scripts must not load ordinary non-frozen `bettable_event_detail_*.csv` rows into `筛选当日可投注赛事`; only `bettable_signal_freeze_*.csv`, `today_page_bettable_events_*`, or file names explicitly containing `frozen/freeze` may drive frozen historical bettable filters.
