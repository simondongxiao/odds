...
import football_decision_engine as decision_engine
...
def canonical_tag(tag: object) -> str:
    return intent_engine.canonicalize(str(tag or "")).get("intent_canonical", "")
...
def normalize_region(raw_region: object, league: object) -> str:
    ...
    if text:
        return {"北美系列": "北美", "南美系列": "拉美", "日韩系列": "东亚", "西亚/中亚系列": "海湾", "欧洲五大系列": "欧洲五大", "欧洲非五大系列": "欧洲非五大"}.get(text, text)
    return {"北美系列": "北美", "南美系列": "拉美", "日韩系列": "东亚", "西亚/中亚系列": "海湾", "欧洲五大系列": "欧洲五大", "欧洲非五大系列": "欧洲非五大"}.get(competition_normalizer.normalize(str(league or "")).micro_region, "其他")
...
def decide_match(
    ...
    """Run the legacy historical funnel, then pass its final action through the shared boundary."""
    legacy = _legacy_decide_match(row, settled_history, strategy_history, config)
    old_action = str(legacy.get("动作", NO_BET))
    engine_action = "可投" if old_action in {FORWARD, REVERSE} else "不投"
    inp = decision_engine.MatchDecisionInput(
        match_id=row.match_id,
        list_date=row.date,
        kickoff=row.kickoff,
        decision_at=row.kickoff,
        prior_snapshot_id="sequential-prior",
        odds_snapshot_id="sequential-odds",
        evidence_snapshot_id="sequential-evidence",
        history_cutoff=row.kickoff,
        competition={"league": row.league, "micro_region": row.region},
        market={"state": "未开赛", "water": row.forward_water if old_action == FORWARD else row.reverse_water},
        intent={"mapped_direction": old_action, "candidate_team": ""},
        history={"probability": legacy.get("综合胜率"), "threshold": legacy.get("通过阈值")},
        risk_state={"state": legacy.get("风控状态", "")},
        supplementary={},
        existing_legacy_decision={
            "action": engine_action,
            "mode": old_action if engine_action == "可投" else "",
            "rate": legacy.get("综合胜率"),
            "threshold": legacy.get("通过阈值"),
            "risk_state": legacy.get("风控状态", ""),
            "reason": legacy.get("拦截原因", ""),
        },
    )
    result = decision_engine.decide(inp)
    legacy["统一决策ID"] = result.decision_id
    legacy["统一决策动作"] = result.action
    legacy["统一决策方向"] = result.final_direction
    legacy["统一规则版本"] = result.rule_version
    if result.action == "不投" and engine_action == "可投":
        legacy["动作"] = NO_BET
        legacy["拦截阶段"] = "统一决策边界"
        legacy["拦截原因"] = result.blocking_gate or "统一决策边界拦截"
    return legacy
