...
def load_scores() -> dict[tuple[str, str, str], dict[str, str]]:
    ...
    for row in titan_over_score_rows():
        ...
        if match_id:
            scores[("match_id", match_id, "")] = payload
    for path in sorted((ROOT / "ledger").glob("verified_score_supplement_*.json")):
        for mid, row in json.loads(path.read_text(encoding="utf-8")).items():
            if row.get("state") != "-1" or not str(row.get("home_score", "")).isdigit() or not str(row.get("away_score", "")).isdigit():
                continue
            old = scores.get(("match_id", mid, ""), {})
            home = row.get("home_cn") or old.get("home")
            away = row.get("away_cn") or old.get("away")
            if home and away:
                scores[("match_id", mid, "")] = dict(home=home, away=away, home_score=row["home_score"], away_score=row["away_score"], status="Final", source=row.get("score_snapshot", str(path)))
    return scores
