...
def actual_team_flow_share(row: dict[str, str]) -> dict[str, object]:
    ...
    if flow_rows and not all(item.get("same_ah_market_verified") is True for item in flow_rows):
        return {"home": None, "away": None, "draw": None,
                "basis": "胜平负累计成交仅作观察；同档亚盘及同时间窗资金未验证"}
...
def flow_overlay_fields(row: dict[str, str]) -> dict[str, str]:
    ...
    if actual_missing:
        return {
            ...
            "实际资金流向": "资金字段缺口",
            "目标侧水位甜头": water_status,
            "意图成败": "同档亚盘资金未验证-胜平负成交仅观察；沿用亚盘EV框架",
            "资金流修正方向": "不因资金流修正",
            "资金流修正球队": "",
            "资金流来源": "Chuqi必发胜平负累计成交，非同档亚盘资金",
...
def main() -> int:
    ...
    history_files = [
        p for p in (
            ...
            ROOT / "ledger" / f"micro_region_tag_edge_*.csv",
            ROOT / "ledger" / f"asian_intent_line_tag_matrix_*.csv",
            ROOT / "ledger" / f"micro_region_risk_state_*.csv",
        ) for p in sorted(ROOT.glob(str(p.relative_to(ROOT))))[-1:]
    ]
    rebuild_prior = "--rebuild-prior" in sys.argv
    prior = versioning.ensure_prior_snapshot(
        TODAY.isoformat(),
        history_cutoff=(TODAY - dt.timedelta(days=1)).isoformat(),
        rule_version="legacy-daily-funnel+v4-step2a-lock",
        tag_history={"source": "frozen-at-first-strict-refresh"},
        micro_history={"source": "frozen-at-first-strict-refresh"},
        line_tag_history={"source": "frozen-at-first-strict-refresh"},
        risk_inputs={"source": "frozen-at-first-strict-refresh"},
        source_files=[snapshot, *history_files],
        rebuild=rebuild_prior,
    )
    decision_write = versioning.record_current_decision_file(TODAY.isoformat(), prior)
    if int(decision_write.get("drifts", 0)):
        drift_path = versioning.write_drift_audit(TODAY.isoformat(), [{"drift_reason": "decision writer detected drift", "status": "ERROR_DECISION_DRIFT"}])
        raise RuntimeError(f"ERROR_DECISION_DRIFT; daily publish stopped; audit={drift_path}")
    existing_today_keys = {
    ...
    frozen_lock_available = (ROOT / "ledger" / f"dashboard_plan_lock_{TODAY:%Y%m%d}.json").exists()
    if frozen_lock_available:
        import finalize_frozen_daily
        finalize_frozen_daily.main()
    ...
    if bettable_stats_script.exists() and not frozen_lock_available:
