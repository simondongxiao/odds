...
def detail_lineup(detail: dict[str, str]) -> str:
    ...
    return (
        ...
        f"主队首发 {detail.get('home_starting','')}；"
        f"客队首发 {detail.get('away_starting','')}；"
        f"主队替补 {detail.get('home_bench','') or '未提供'}；客队替补 {detail.get('away_bench','') or '未提供'}"
    )
...
def detail_injury(detail: dict[str, str]) -> str:
    if not detail:
        return "伤停未接入/待核"
    home = detail.get("home_injuries", "")
    away = detail.get("away_injuries", "")
    if home or away:
...
def odds_summary(
    ...
    return {
        ...
        "source_match": f"{strip_html(row.get('home_cn', ''))} vs {strip_html(row.get('away_cn', ''))}",
        "ah": ah,
        "euro": euro,
        ...
        cards.append(
            {
                ...
                "form_source": f"球探Analysis：{detail.get('recent_form_summary','')}" if detail.get("recent_form_summary") else "未接入/待核：待接近5场和主客场拆分",
...
def apply_saved_dashboard_decisions(cards: list[dict]) -> list[dict]:
    ...
    prior_paths = sorted((ROOT / "ledger").glob(f"ah_decisions_before_refresh_{TODAY:%Y%m%d}_*.json"))
    prior_path = prior_paths[-1] if prior_paths else None
    if prior_path is not None:
        for record in json.loads(prior_path.read_text(encoding="utf-8"))["records"]:
            old = record["card"]
            ...
            if not fresh:
                continue
            try:
                kickoff = dt.datetime.strptime(old.get("time", ""), "%Y-%m-%d %H:%M")
            except ValueError:
                continue
            if kickoff > dt.datetime.now() and str(fresh.get("state")) == "0":
                previous = record["decision"]
                if previous.get("action") not in ("可投", "半仓可投"):
                    anchor = previous.get("previousDecision") or old.get("previous_saved_decision") or {}
                    if anchor.get("action") in ("可投", "半仓可投"):
                        previous = anchor
                fresh["previous_saved_decision"] = previous
                previous_team = previous.get("team", "").split("（", 1)[0]
                teams = old.get("match", "").split(" vs ")
                fresh["previous_selected_index"] = teams.index(previous_team) if previous_team in teams else -1
                continue
            merged = dict(old)
            for field in ("score", "display_score", "state", "state_label", "display_status", "result", "pnl", "status", "grade", "error", "update"):
                if field in fresh:
                    merged[field] = fresh[field]
            for field in ("form_source", "h2h_source", "lineup_source", "motivation_source", "lineup", "injury"):
                if fresh.get(field):
                    merged[field] = fresh[field]
            merged["saved_skill_decision"] = record["decision"]
            merged["saved_ev_badge"] = record["badge"]
    ...
    for gate_file in sorted((ROOT / "ledger").glob("current_v3_gate_*.json")):
        ...
        for card in by_key.values():
            ...
            if gate and not card.get("saved_skill_decision"):
                card["supplementary_evidence_gap"] = gate.get("reason", "")
                card["v3_gate_status"] = "EVIDENCE_INCOMPLETE"
                # Supplementary gaps do not replace the per-match AH decision.
                if gate.get("verified_hard_veto") is True and gate.get("action") == "不投":
                    card["saved_skill_decision"] = gate
    for supplement in sorted((ROOT / "ledger").glob("verified_score_supplement_*.json")):
        scores = json.loads(supplement.read_text(encoding="utf-8"))
        for card in by_key.values():
            score = scores.get(str(card.get("match_id", "")))
            if score:
                if str(card.get("state")) == "-1" and score.get("state") != "-1":
                    continue
                value = score["home_score"] + "-" + score["away_score"] if score.get("home_score", "") != "" and score.get("away_score", "") != "" else "待赛" if score.get("state") == "0" else "待核"
                state = score.get("state", "")
                card.update(score=value, display_score=value, state=state, state_label=titan_state_name(state), display_status=titan_state_name(state))
    settled_file = ROOT / "ledger" / f"bettable_event_detail_{TODAY.isoformat()}.csv"
    if settled_file.exists():
        with settled_file.open(encoding="utf-8-sig", newline="") as f:
            settled = {r.get("原模拟ID"):r for r in csv.DictReader(f) if r.get("已结算") == "是" and r.get("原模拟ID")}
        for card in by_key.values():
            result = settled.get(card.get("sim_id"))
            if result:
...
<body>
  ...
  <details class="note" id="riskAudit20260907" style="margin:8px 16px;">
  ...
  <div class="note" id="strictUpdate20260908">补充信息缺口不自动否决亚盘：可投筛选按标签历史、微观组合、水位及风控逐场判断；独立Delta未验证不等于不投。周末不加严，历史结论保持冻结。</div>
  <main class="shell">
    <aside class="left">
...
function frameworkDecision(r, cell = null, options = {{}}) {{
  ...
  if (r.supplementary_evidence_gap) details.push(`补充信息缺口（不自动否决）：${{r.supplementary_evidence_gap}}。`);
...
function fourStepCalculation(r, cell, decision) {{
  const tag = String(r.intent_tag || "");
  const edge = decision.edge || microEdgeEntry(r);
  const policy = r.top5_policy?.is_top5 ? r.top5_policy : null;
  const rows = [
    `1）意图及同盘口历史EV：${{r.intent_line_bucket || "缺盘口档位"}} + ${{tag || "缺标签"}}；正向=${{positiveTeamText(r, "forward")}}，反向=${{positiveTeamText(r, "reverse")}}。` +
    (cell ? `同档样本${{cell.sample}}；正向胜率${{cell.forward_rate}}/收益${{signed(cell.forward_pnl)}}，反向胜率${{cell.reverse_rate}}/收益${{signed(cell.reverse_pnl)}}。` : "同档样本未生成，不虚构EV。"),
    `2）标签整体表现：${{tagPerformanceLabel(tag)}}`
  ];
  if (policy) {{
    rows.push(`3）五大地区赛事分支：${{top5StatsLine(policy.league || "同赛事", policy.league_stats)}}；${{top5StatsLine("同赛事同盘口+标签", policy.line_stats)}}。原版采用同向跟随、异向选历史胜率较高方，不称作贝叶斯加权；本场依据：${{policy.selected_reason || "样本待核"}}。`);
  }} else if (edge) {{
    rows.push(`3）微观组合加权：${{r.micro_region}} + ${{tag}}；局部n=${{edge["样本数"] || 0}}，全局M=${{edge["标签总样本"] || 0}}；局部正/反=${{edge["正向有效胜率"] || "缺失"}}/${{edge["反向有效胜率"] || "缺失"}}，全局正/反=${{edge["标签正向胜率"] || "缺失"}}/${{edge["标签反向胜率"] || "缺失"}}。方向分歧时按同一方向的(n×局部胜率+M×全局胜率)/(n+M)；已存综合胜率=${{edge["贝叶斯综合胜率"] || "未生成"}}。微观板块不等于单一联赛。`);
  }} else {{
    rows.push(`3）微观组合：${{r.micro_region || "未分类"}} + ${{tag}} 无可用样本，不能计算加权胜率。`);
  }}
  const mode = decision.mode || "none";
  const water = Number(decision.water || selectedWater(r, mode) || 0);
  const rate = Number(decision.rate || 0);
  const threshold = water > 0 ? breakevenThreshold(water) : 0;
  const veto = sameLineVeto(cell, mode);
  const risk = riskEntry(r);
  rows.push(`4）价格及风控：选中方向=${{decision.team}}；水位=${{water > 0 ? water.toFixed(4) : "缺失"}}；阈值=1/(1+水位)+2%=${{threshold ? pct(threshold) : "无法计算"}}；采用胜率=${{rate ? pct(rate) : "未形成"}}；同档检查=${{veto || "未触发否决/样本不足不触发"}}；板块风控=${{risk?.["风控状态"] || "未生成"}}。最终：${{decision.action}}；${{decision.reason}}。`);
  if (decision.reverseAlert) rows.push("小样本反向警戒已触发：先修正候选方向，再检查选中方向的水位与风控，不是直接放行。");
  rows.push("Kelly仓位校验：先确定方向及适用概率，再按选中球队的盘口、水位计算。旧版红框未调用独立Kelly模块，不能称为已通过Kelly；全赢/全输二项合约使用f=(水位×p-(1-p))/水位，有走水或半赢半输的亚盘须用完整结算分布计算。未完成精确Kelly或缺本金/限额时仅标仓位待核，不因此取消已通过EV的可投计划，也不虚构下单金额。");
  if (r.supplementary_evidence_gap) rows.push(`补充信息缺口（不自动否决）：${{r.supplementary_evidence_gap}}。`);
...
function top5PolicyBadge(r, cell = null, decisionOverride = null) {{
  ...
  const detail = fourStepCalculation(r, cell, decision);
  const sameLine = cell
