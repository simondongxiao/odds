...
from .common import *
from .snapshots import collect,paths_for,windows,in_primary_freeze_window,primary_window_status
...
def tick(day):
    ...
    for mid,rosterrow in roster.items():
        ...
        if path.exists():
            frozen=read(path);status='FROZEN_PRIMARY_4H' if frozen.get('execution_policy_version') else 'FROZEN_PRIMARY_T30';cards.append({**frozen,'window_status':status});continue
        ...
        if s:
            history=paths_for(day,mid);win=windows(history,current);card.update(snapshot=s,windows={k:{'status':v['status'],'snapshot_id':v['snapshot']['snapshot_id'] if v['snapshot'] else None} for k,v in win.items()},window_status=primary_window_status(s['minutes_to_kickoff']))
            if s['valid_market'] and s['handicap'] and s['euro']:
                ...
                if in_primary_freeze_window(s['minutes_to_kickoff']) and 0<=(at-clock(s['quote_at'])).total_seconds()/60<=config('snapshot_policy')['max_quote_age_minutes']:
                    ...
                    if c is not None and c.get('quote_at') and abs((clock(c['quote_at'])-clock(s['quote_at'])).total_seconds())<2 and clock(c['decision_at'])<clock(s['kickoff_at']) and d.get('grade') in 'ABC':
                        frozen=freeze(s,f,p,d,model,at,c);card.update(frozen,window_status='FROZEN_PRIMARY_4H')
