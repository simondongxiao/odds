...
# git() trims the whole output, so the first porcelain line can lose its
# leading worktree-status space. Parse from column two and trim whitespace.
if any(not line[2:].lstrip().replace('"','').startswith('v41/') for line in status.splitlines()):raise ValueError('UNRELATED_GIT_CHANGES_DO_NOT_STAGE')
...
if staged:git('commit','-m','Update V4.1 Shadow execution window and daily freeze')
