...
repo=WORKSPACE/'outputs/football_odds_trader/github_publish/odds';commit=subprocess.check_output(['git','-C',str(repo),'rev-parse','HEAD'],text=True).strip();proxy=os.environ.get('V41_HTTP_PROXY','');proxies={'http':proxy,'https':proxy} if proxy else {};session=requests.Session();session.trust_env=False
r=session.get('https://api.github.com/repos/simondongxiao/odds/actions/runs',params={'head_sha':commit,'per_page':5},proxies=proxies,timeout=30);r.raise_for_status();runs=r.json()['workflow_runs'];run=next((r for r in runs if r['head_sha']==commit),None)
...
def verify(rel):
    url='https://simondongxiao.github.io/odds/'+rel+'?v='+commit[:7];r=session.get(url,proxies=proxies,timeout=60);r.raise_for_status();return {'url':url,'status':r.status_code,'content_matches':r.content.decode('utf-8-sig').replace('\r\n','\n')==(repo/rel).read_text(encoding='utf-8-sig').replace('\r\n','\n')}
