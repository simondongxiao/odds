import numpy as np
from .common import *
from .snapshots import in_primary_freeze_window
...
def decide(pred,s,f,as_of):
    ...
    if not in_primary_freeze_window(s['minutes_to_kickoff']):reasons.append('NOT_PRIMARY_4H_WINDOW')
    ...
    if not reasons:
        for grade,g in cfg['grades'].items():
            if value>g['ev'] and out['EV_p10']>g['p10'] and out['P_EV_positive']>=g['positive'] and pred['train_support']>=g['train_cell'] and pred['calibration_support']>=g['calibration_cell'] and f['feature_completeness']>=g['completeness'] and width<=g['max_width'] and edge>=g['probability_edge']:
        ...
        if pred['calibration_support']<cfg['grades']['C']['calibration_cell']:reasons.append('INSUFFICIENT_CALIBRATION_SUPPORT')
        out.update(grade='C',action='SHADOW CANDIDATE',strict_grade='N',grade_policy='USER_FORCED_ABC_FALLBACK_4H_20260920',decision_reason=['FORCED_C_OBSERVATION_USER_RULE',*reasons]);return out
...
def freeze(s,f,pred,decision,model,as_of,control=None):
    ...
    if not in_primary_freeze_window(s['minutes_to_kickoff']):raise ValueError('PRIMARY_4H_WINDOW_REQUIRED')
    ...
    row={'model_version':model['model_version'],'model_artifact_id':model['artifact_id'],'execution_policy_version':execution['version'],'primary_freeze_window':execution['primary_freeze_window'],'role':'CHALLENGER','status':'SHADOW ONLY','real_money':False,'frozen_at':clock(as_of).isoformat(),'list_date':s['list_date'],'match_id':s['match_id'],'snapshot':s,'features':f,'probability':{k:v for k,v in pred.items() if k!='ensemble'},'decision':decision,'control':control}
