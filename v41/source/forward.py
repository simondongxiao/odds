...
def reconcile():
    ...
    for path in sorted((ROOT/'decisions').glob('*/*.json')):
        ...
        rows.append(row)
    fields=['date','match_id','league','region','kickoff','frozen_at','handicap','intent','time_bucket','v4_side','v4_grade','v4_ev','v4_probability','v4_result','v4_pnl','v41_side','v41_grade','v41_probability','v41_ev','v41_ev_p10','v41_result','v41_pnl','same_side','different_side','comparison_group','decision_id','settlement_status']
...
def performance(rows,prefix):
    candidates=[r for r in rows if r.get(prefix+'_grade') in ['A','B','C']];settled=sorted([r for r in candidates if r.get(prefix+'_result') in STATES],key=lambda r:(r['kickoff'],r['match_id']));pnls=[r[prefix+'_pnl'] for r in settled];outcomes=[r[prefix+'_result'] for r in settled];counts={s:outcomes.count(s) for s in STATES};den=counts['W']+.5*counts['HW']+counts['L']+.5*counts['HL'];curve=np.r_[0,np.cumsum(pnls)];dd=float(np.max(np.maximum.accumulate(curve)-curve))
